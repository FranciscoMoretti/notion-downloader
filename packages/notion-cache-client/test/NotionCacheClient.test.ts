import { buildNotionCacheClientWithFixture } from "@notion-fixtures/fixtures/fixtureUtils"
...
const blocksObjectsData = Object.values(
  sampleSiteReader.cache.blockObjectsCache
).map((block) => block.data)
const pageObjectsData = Object.values(
  sampleSiteReader.cache.pageObjectsCache
).map((block) => block.data)
const databaseObjectsData = Object.values(
  sampleSiteReader.cache.databaseObjectsCache
).map((block) => block.data)
...
describe("NotionCacheClient - block", () => {
  ...
  it("gets a miss from cache and uses notion-api for non-existent block", async () => {
    ...
    // Create a spy function with vitest
    const spy = vi
      .spyOn(notionClient.notionClient.blocks, "retrieve")
  ...
  it("after getting the block once, it gets it from the cache the next time", async () => {
    ...
    const block1 = await notionClient.blocks.retrieve({
      block_id: blockResponse.id,
    })
    const block2 = await notionClient.blocks.retrieve({
      block_id: blockResponse.id,
    })
...
describe("NotionCacheClient - block children", () => {
  it("gets block children from cache", async () => {
    ...
    const blockChildren = await notionClient.blocks.children.list({
      block_id: blockWithChildren.id,
    })
    expect(blockChildren).toStrictEqual(blockChildrenOfBlockResponse)
  })
  ...
  it("gets a miss from cache and uses notion-api for non-existent block", async () => {
    ...
    // Create a spy function with vitest
    const spy = vi
      .spyOn(notionClient.notionClient.blocks.children, "list")
      .mockResolvedValue(blockChildrenOfBlockResponse)
  ...
  it("after getting the block children once, it gets it from the cache the next time", async () => {
    ...
    const spy = vi
      .spyOn(notionClient.notionClient.blocks.children, "list")
      .mockResolvedValueOnce(blockChildrenOfBlockResponse)
    const blockChildren1 = await notionClient.blocks.children.list({
      block_id: blockWithChildren.id,
    })
    const blockChildren2 = await notionClient.blocks.children.list({
      block_id: blockWithChildren.id,
    })
...
describe("NotionCacheClient - page", () => {
  ...
  it("gets a miss from cache and uses notion-api for non-existent page", async () => {
    ...
    // Create a spy function with vitest
    const spy = vi
      .spyOn(notionClient.notionClient.pages, "retrieve")
  ...
  it("after getting the page once, it gets it from the cache the next time", async () => {
    ...
    const blockChildren1 = await notionClient.pages.retrieve({
      page_id: pageResponse.id,
    })
    const blockChildren2 = await notionClient.pages.retrieve({
      page_id: pageResponse.id,
    })
    expect(blockChildren1).toStrictEqual(blockChildren2)
    expect(spy).toHaveBeenCalledOnce()
...
describe("NotionCacheClient - database", () => {
  ...
  it("gets a miss from cache and uses notion-api for non-existent database", async () => {
    ...
    // Create a spy function with vitest
    const spy = vi
      .spyOn(notionClient.notionClient.databases, "retrieve")
      .mockResolvedValue(databaseResponse)
    await notionClient.databases.retrieve({
      database_id: "non-existent-database",
    })
    expect(spy).toHaveBeenCalledOnce()
  })
  ...
  it("after getting the database once, it gets it from the cache the next time", async () => {
    ...
    const spy = vi
      .spyOn(notionClient.notionClient.databases, "retrieve")
      .mockResolvedValueOnce(databaseResponse)
    const database1 = await notionClient.databases.retrieve({
      database_id: databaseResponse.id,
    })
    const database2 = await notionClient.databases.retrieve({
      database_id: databaseResponse.id,
    })
...
describe("NotionCacheClient - database children", () => {
  it("gets database children from cache", async () => {
    ...
    const databaseChildren = await notionClient.databases.query({
      database_id: databaseResponse.id,
    })
    expect(databaseChildren).toStrictEqual(
  ...
  it("gets a miss from cache and uses notion-api for non-existent database children", async () => {
    ...
    // Create a spy function with vitest
    const spy = vi
      .spyOn(notionClient.notionClient.databases, "query")
      .mockResolvedValue(pageOrDatabaseChildrenOfDatabaseResponse)
  ...
  it("after getting the database children once, it gets it from the cache the next time", async () => {
    ...
    const spy = vi
      .spyOn(notionClient.notionClient.databases, "query")
      .mockResolvedValueOnce(pageOrDatabaseChildrenOfDatabaseResponse)
    const databaseChildren1 = await notionClient.databases.query({
      database_id: databaseResponse.id,
    })
    const databaseChildren2 = await notionClient.databases.query({
      database_id: databaseResponse.id,
    })
