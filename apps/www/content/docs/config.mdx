...
The `notion-downloader` CLI can be configured using a `notion-downloader.config.json` file in your project directory.

You can create a `notion-downloader.config.json` file by running the following command:

```bash
notion-downloader init
```

This will prompt you for the required configuration options and create a `notion-downloader.config.json` file in your current directory.
